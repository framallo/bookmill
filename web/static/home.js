...
// Verdict glyphs as inline SVG (never emoji), inheriting the chip color.
const VICON = {
...
async function runCheckAll() {
  ...
  for (const j of jobs) {
    ...
    try {
      ...
      else if (warns) { vcls = 'warn'; v = `${warns} warn`; }
      row.innerHTML = caLabel(j) +
        `<a class="verdict ${vcls}" href="/book.html?book=${encodeURIComponent(j.slug)}&lang=${encodeURIComponent(j.lang)}">${VICON[vcls]} ${v}</a>`;
    } catch (e) {
